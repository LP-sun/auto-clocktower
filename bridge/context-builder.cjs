...
function buildContext({actor,task,actions,view,memory,store,profile,runtimeConfig}){
 ...
 control.responseProtocol={...OUTPUT_PROTOCOL,instruction:OUTPUT_PROTOCOL.instruction+' 字段名严格使用 action,message,players,reasoning,communication,memoryUpdate；不要用 target 或 reason。communication 是机器可读的发言元数据，仅描述 message 实际表达的内容，不得加入未说出的秘密；identityClaims 中 subject 和 claimedRole 使用座位号与角色 ID。memoryUpdate 用于跨回合保存私有 beliefs、plan、worlds；无更新时填 null。',example:{action:actions[0],message:'',players:[],reasoning:'简短行动依据。',communication:{intent:'none',identityClaims:[],evidenceRefs:[]},memoryUpdate:null}};
 ...
 const agentParameters=st?undefined:{personality:profile?.personality,skill:profile?.skill,interpretation:'这些连续参数描述你的稳定行为倾向，并非行动指令或概率硬门槛。skill 影响证据校准、记忆复核、投票纪律和伪装一致性；decisionTemperature 越高，合理候选之间越愿意探索。仍须由你结合当前证据完成推理。'};
 const sections={script:st?undefined:undefined,scriptIndex,character:st?undefined:getCharacterStrategy(view.self.role),roleStrategyNotice:st?undefined:'以下 Role-Specific Strategic Guidance 是策略启发，不是规则或强制动作；根据当前局面自行判断。当前上下文只展开你的角色规则；其他角色先通过 scriptIndex 判断类别，不能据此推断本局存在或玩家身份。',agentParameters,state:view,memory:semantic,recent,control};
 ...
 const configured=runtimeConfig?.limits?.contextTokens;const budget=Number(process.env.BOTC_CONTEXT_BUDGET || (st?configured?.storyteller:light?configured?.light:configured?.player) || (st?14000:light?4000:12000));
