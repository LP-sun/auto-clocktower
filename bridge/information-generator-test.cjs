...
test('pair generation uses ST tuple, no random draft; offers other true Townsfolk',async()=>{
 const c=ctx(['washerwoman','soldier','chef','imp']);let called=0;
 setInformationGenerator(async(current,choices)=>{assert.equal(current,c);called++;assert(choices.every(x=>x.p1!=='P0'&&x.p2!=='P0'));return choices.findIndex(x=>x.p1==='P1'&&x.p2==='P2'&&x.role==='soldier');});
...
test('invalid ST choices fail closed',async()=>{setInformationGenerator(async()=>-1);await assert.rejects(generatePair(ctx(['washerwoman','soldier','chef','imp'])),/invalid information/);});
