...
export async function killPlayer(
  ...
  const record: DeathRecord = {
    poisonedAtDeath: ps.tags.has("poisoned"),
  ...
  ps.alive = false;
  ps.death = record;
  for (const target of runtime.playerStates) {
    if (target.poisonSourceId === playerId) { target.tags.delete("poisoned"); delete target.poisonSourceId; }
  }
  updateGame(state);
