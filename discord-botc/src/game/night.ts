import { decideLegal } from "./discretion";
import { generatePair, generateRoleInformation } from './informationGenerator';
...
// Optional headless ST review, after rules resolve and before private delivery.
let automatedInfoReview: ((state: GameState) => Promise<void>) | undefined;
export function setAutomatedInfoReview(review: (state: GameState) => Promise<void>): void {
  automatedInfoReview = review;
}
...
function runActionResolves(
  ...
  const entries = [...session.responses.entries()].sort((a, b) =>
    order.indexOf(getPlayerState(runtime, a[0])!.effectiveRole.id) -
    order.indexOf(getPlayerState(runtime, b[0])!.effectiveRole.id));
  for (const [playerId, values] of entries) {
    ...
    if (actorPs.tags.has("poisoned")) continue;
 ...
 */
async function applyKillIntent(state: GameState): Promise<boolean> {
  const runtime = ensureRuntime(state);
  ...
  const impPs = runtime.playerStates.find((ps) => ps.role.id === "imp" && ps.alive);
  ...
  if (targetPs.role.id === "soldier" && !targetPs.tags.has("poisoned")) return impKilledSelf;
...
function promoteImpOnSelfKill(state: GameState): string | null {
  ...
  for (const target of runtime.playerStates) { if (target.poisonSourceId === newImpPs.player.userId) { target.tags.delete("poisoned"); delete target.poisonSourceId; } }
...
async function proceedAfterResolution(
  ...
  if (state.mode === "manual") {
    ...
    if (automatedInfoReview) await automatedInfoReview(state);
...
async function handleDeathNarrativeDM(
  ...
  if (kind === "ravenkeeper") {
    ...
    const generatedRole = await generateRoleInformation(buildCtx(state,client,rkPs!,session.nightNumber,session.responses,lang),target.userId);
    if (generatedRole !== undefined) {
      shownRoleId = generatedRole;
    } else if (falsified) {
