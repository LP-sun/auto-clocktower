...
let policy: ((decision: RegistrationDecision) => Registration) | undefined;
...
export function registersAs(role: Role, target: DetectionTarget, subject?: PlayerRuntimeState): boolean {
  ...
  if (policy && legalOptions.length > 1) {
    try { const result = policy({ type:"registration",role:role.id,player:subject?.player.userId,target,legalOptions:[...legalOptions] }); if (legalOptions.includes(result)) chosen=result; } catch { /* legal truthful fallback */ }
  }
